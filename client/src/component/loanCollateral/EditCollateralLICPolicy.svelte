<script>
  ...
  export let licPolicy;

  let dataLoaded = false;

  const dispatch = createEventDispatcher();
  console.log("licPolicy: ", licPolicy);

  const onSave = () => {
    console.log("licPolicy being saved is: ", licPolicy);
    clearErrors(licErrors);

    if (!onSubmit()) return false;
    console.log("dispatch called");
    dispatch("saveLICPolicy", licPolicy);
  ...
  const onInput = (field, type) => {
    clearErrors(licErrors);
    console.log("LIC field : ", field, "value: ", licPolicy[field]);

    resLICValidate = LICSuite(licPolicy, field);
  ...
  const onSubmit = () => {
    clearErrors(licErrors);
    resLICValidate = LICSuite(licPolicy, "submit");
    console.log("licErrors: ", resLICValidate.errors);
