<!-- ------- -->
<script>
  import { browser } from "$app/environment";
  import { masters } from "$appstore/store";
  import { onMount } from "svelte";
  import { FloatingLabelInput, Helper } from "flowbite-svelte";

  export let survey_no = "";
  export let dist_id = 0;
  export let taluka_id = 0;
  export let village = "";
  export let pincode = "";
  export let maj_unit = "";
  export let min_unit = "";
  export let area_unit = "";
  export let property_value = "";
  export let sub_property = "";
  export let readonly = false;

  export let data;
  export let errors;
  export let onInput;
  export let alertMsg;

  let districts = [];
  let talukas = [];
  onMount(() => {
    if (browser) {
      districts = [...$masters.m_district];
      if (dist_id) {
        talukas = [...$masters.m_taluka.filter((e) => e.dist_id == dist_id)];
      }
    }
  });
  $: talukas = [...$masters.m_taluka.filter((e) => e.dist_id == dist_id)];
</script>

<!-- Address -->
<div class="relative border border-1 border-gray-200 p-3 mb-6 mt-3 bg-gray-100">
  <div class="flex flex-col my-3">
    <div
      class="absolute text-gray-600 text-xs font-thin -top-3 bg-gray-200 rounded p-1"
    >
      Property Address/मालमत्तेचा पत्ता
    </div>
    <label for="address" class="block text-sm font-medium text-gray-700">
      Survey No/सर्वे नंबर<span class="text-red-500">*</span>
    </label>
    <div>
      <input
        on:input={(e) => {
          data.survey_no = e.target.value.toUpperCase();
          // data = data
          onInput("survey_no");
        }}
        type="text"
        id="address"
        name="address"
        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
        placeholder="Enter Survey No."
        bind:value={survey_no}
        {readonly}
      />
      {#if errors["survey_no"]}
        {#if errors.survey_no}
          <Helper color="red" class="absolute">
            {errors.survey_no}
          </Helper>
        {/if}
      {/if}
    </div>
  </div>

  <div class="flex items-center justify-between space-x-4 my-3">
    <div class="flex flex-col">
      <div class="block w-full">
        <label
          for="district"
          class="block mb-2 text-sm font-medium text-gray-600 w-full"
          >District/जिल्हा<span class="text-red-500">*</span></label
        >
        <select
          id="district"
          class="h-12 border border-gray-300 text-gray-600 text-base rounded-lg block w-full py-2.5 px-4 focus:outline-none"
          disabled={readonly}
          on:change={(e) => {
            taluka_id = 0;
            dist_id = e.target.value;
          }}
        >
          <option selected disabled>District</option>
          {#each districts as district}
            {#if district.dist_id == dist_id}
              <option value={district.dist_id} selected>
                {district.eng_name}&nbsp;-&nbsp;{district.dev_name}
              </option>
            {:else}
              <option value={district.dist_id}>
                {district.eng_name}&nbsp;-&nbsp;{district.dev_name}
              </option>
            {/if}
          {/each}
        </select>
      </div>
    </div>

    <div class="flex flex-col">
      <div class="block w-full">
        <label
          for="taluka"
          class="block mb-2 text-sm font-medium text-gray-600 w-full"
          >Taluka/तालुका<span class="text-red-500">*</span></label
        >
        <select
          id="taluka"
          disabled={readonly}
          on:change={(e) => {
            taluka_id = e.target.value;
          }}
          class="h-12 border border-gray-300 text-gray-600 text-base rounded-lg block w-full py-2.5 px-4 focus:outline-none"
        >
          <option value="0" selected>Taluka</option>
          {#each talukas as taluka}
            {#if taluka.taluka_id == taluka_id}
              <option value={taluka.taluka_id} selected>
                {taluka.eng_name}&nbsp;-&nbsp;{taluka.dev_name}
              </option>
            {:else}
              <option value={taluka.taluka_id}>
                {taluka.eng_name}&nbsp;-&nbsp;{taluka.dev_name}
              </option>
            {/if}
          {/each}
        </select>
      </div>
    </div>
  </div>

  <div class="flex items-center justify-between space-x-4 my-3">
    <div class="flex flex-col">
      <label for="village" class="block text-sm font-medium text-gray-700">
        Village/गांव<span class="text-red-500">*</span>
      </label>
      <div>
        <input
          on:input={(e) => {
            data.village = e.target.value.toUpperCase();
            // data = data
            onInput("village");
          }}
          type="text"
          id="village"
          name="village"
          class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
          placeholder="Enter City/Town/Village of property"
          bind:value={village}
          {readonly}
        />
        {#if errors["village"]}
          {#if errors.village}
            <Helper color="red" class="absolute w-60">
              {errors.village}
            </Helper>
          {/if}
        {/if}
      </div>
    </div>
    <div class="flex flex-col">
      <label for="pincode" class="block text-sm font-medium text-gray-700">
        PIN code/पिन कोड<span class="text-red-500">*</span>
      </label>
      <div>
        <input
          on:input={(e) => {
            data.pincode = e.target.value.toUpperCase();
            // data = data
            onInput("pincode");
          }}
          type="text"
          id="pincode"
          name="pincode"
          class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
          placeholder="Enter PIN code"
          bind:value={pincode}
          {readonly}
        />
        {#if errors["pincode"]}
          {#if errors.pincode}
            <Helper color="red" class="absolute w-60">
              {errors.pincode}
            </Helper>
          {/if}
        {/if}
      </div>
    </div>
  </div>
</div>
<!-- Area -->
<div
  class="relative flex space-x-4 my-3 mb-8 border border-1 border-gray-200 p-3 bg-gray-100"
>
  <div
    class="absolute text-gray-600 text-xs font-thin -top-3 bg-gray-200 rounded p-1"
  >
    Property Value and Area/मालमत्तेची किंमत रुपये मध्ये
  </div>
  <!-- Area Units -->
  <div>
    <label for="area-unit" class="block text-sm font-medium text-gray-700">
      Units/युनिट्स<span class="text-red-500">*</span>
    </label>
    <select
      id="area-unit"
      name="area-unit"
      class="mt-1 block w-full px-1 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
      bind:value={data.area_unit}
    >
      <option value="sqft">hectors</option>
      <option value="sqmtr">acres</option>
    </select>
  </div>
  <div>
    <label for="maj_unit" class="block text-sm font-medium text-gray-700">
      Hectors/acers/हेक्टर/एकर<span class="text-red-500">*</span>
    </label>
    <div>
      <input
        on:input={(e) => {
          data.maj_unit = e.target.value.toUpperCase();
          // data = data
          onInput("maj_unit");
        }}
        type="text"
        id="maj_unit"
        name="maj_unit"
        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
        placeholder="Enter Area"
        bind:value={maj_unit}
        {readonly}
      />
      {#if errors["maj_unit"]}
        {#if errors.maj_unit}
          <Helper color="red" class="absolute w-40">
            {errors.maj_unit}
          </Helper>
        {/if}
      {/if}
    </div>
  </div>
  <div>
    <label for="min_unit" class="block text-sm font-medium text-gray-700">
      R/guntha/आर/गुंठा<span class="text-red-500">*</span>
    </label>
    <div>
      <input
        on:input={(e) => {
          data.min_unit = e.target.value.toUpperCase();
          // data = data
          onInput("min_unit");
        }}
        type="text"
        id="min_unit"
        name="min_unit"
        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
        placeholder="Enter Area"
        bind:value={min_unit}
        {readonly}
      />
      {#if errors["min_unit"]}
        {#if errors.min_unit}
          <Helper color="red" class="absolute w-40">
            {errors.min_unit}
          </Helper>
        {/if}
      {/if}
    </div>
  </div>

  <div>
    <label for="property-value" class="block text-sm font-medium text-gray-700">
      Property Value in Rs/मालमत्तेची किंमत रुपये मध्ये<span
        class="text-red-500">*</span
      >
    </label>
    <div>
      <input
        on:input={(e) => {
          data.property_value = e.target.value.toUpperCase();
          // data = data
          onInput("property_value");
        }}
        type="text"
        id="property-value"
        name="property-value"
        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
        placeholder="Enter property value"
        bind:value={property_value}
        {readonly}
      />
      {#if errors["property_value"]}
        {#if errors.property_value}
          <Helper color="red" class="absolute w-48">
            {errors.property_value}
          </Helper>
        {/if}
      {/if}
    </div>
  </div>
</div>
<!-- Property Value -->

<!-- ------- -->
