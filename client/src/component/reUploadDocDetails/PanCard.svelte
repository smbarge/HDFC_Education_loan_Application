<script>
  ...
  const init = async () => {
    ...
    if (
      details.find(
        (e) => e.field_name == "pan" && e.table_name == "personalDetails"
      )
    ) {
      panCardDetails = {
        pan: details.find(
  ...
  function areDetailsEqual(panCardDetails, personalDetails) {
    if (details.find((e) => e.field_name == "pan")) {
      if (
        panCardDetails["pan"] !==
        details.find((e) => e.field_name == "pan")?.value
      ) {
        return false;
      }
      return true;
      ...
      for (const key in panCardDetails) {
        if (panCardDetails[key] !== personalDetails[key]) {
          return false;
        }
      }
      return true;
          ...
          <div>
            <div class="pb-4">
              <label
                ...
                >Enter Applicant Pan Number</label
              >
              <input
                id="responseInput1"
                type="text"
