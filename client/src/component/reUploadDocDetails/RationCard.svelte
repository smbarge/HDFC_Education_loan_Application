<script>
  ...
  const init = async () => {
    ...
    if (details.find((e) => e.field_name == "ration_card_number")) {
      rationCardDetails = {
        ration_card_number: details.find(
          (e) => e.field_name == "ration_card_number"
        )?.value,
      };
    } else {
      rationCardDetails = {
        ration_card_number: application.personalDetails.ration_card_number,
      };
    }
  };
  onMount(async () => {
  ...
  const onSubmit = async () => {
    console.log("ration card details", rationCardDetails);
  ...
  function areDetailsEqual(rationCardDetails, personalDetails) {
    if (details.find((e) => e.field_name == "ration_card_number")) {
      if (
        rationCardDetails["ration_card_number"] !==
        details.find((e) => e.field_name == "ration_card_number")?.value
      )
        return false;
      return true;
    } else {
      for (const key in rationCardDetails) {
        if (rationCardDetails[key] !== personalDetails[key]) {
          return false;
        }
      }
      return true;
    ...
    <div class="flex-1">
      ...
      >
        <h1 class="text-xl font-semibold text-left text-Calypso-500">
          1. Applicant Ration Card Details
        </h1>
      </button>
      <div class="mt-6">
          ...
          <div>
            <div class="pb-4">
              <label
                ...
                >Enter Applicant Ration Card Number</label
              >
              <input
                id="responseInput1"
                type="text"
                bind:value={rationCardDetails.ration_card_number}
