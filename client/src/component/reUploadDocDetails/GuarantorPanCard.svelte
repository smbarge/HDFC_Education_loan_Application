<script>
  ...
  import Guarantor from "../guarantor/Guarantor.svelte";
  ...
  const init = async () => {
    ...
    if (
      details.find(
        (e) =>
          e.field_name == "pan" && e.table_name == "guarantor_personal_details"
      )
    ) {
      panCardDetails = {
        pan: details.find((e) => e.field_name == "pan" && e.table_name == "guarantor_personal_details")?.value,
      };
    } else {
      panCardDetails = {
        pan: application.guarantorPersonalDetails.pan,
      };
    }
  };
  onMount(async () => {
  ...
  const onSubmit = async () => {
    console.log("aadhardDetails", panCardDetails);
  ...
  function areDetailsEqual(panCardDetails, guarantorPersonalDetails) {
    if (
      details.find(
        (e) =>
          e.field_name == "pan" && e.table_name == "guarantor_personal_details"
      ...
      for (const key in panCardDetails) {
        if (panCardDetails[key] !== guarantorPersonalDetails[key]) {
          return false;
        }
      }
      return true;
          ...
          <div>
            <div class="pb-4">
              <label
                ...
                >Enter Guarantor Pan Number (Guarantor Name: {application
                  .guarantorPersonalDetails.name})</label
              >
              <input
                id="responseInput1"
                type="text"
                bind:value={panCardDetails.pan}
      ...
      <div>
        <div class="pb-4">
          <label
            for="responseInput1"
            class="block text-lg font-medium text-gray-700"
            >Enter Guarantor Pan Number (Guarantor Name: {application.guarantorPersonalDetails.name})</label
          >
          <input
            id="responseInput1"
            type="text"
